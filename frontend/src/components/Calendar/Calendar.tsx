...
function Calendar({ selectedDate, setSelectedDate }: { selectedDate: Date; setSelectedDate: (date: Date) => void }) {
  ...
  return (
    <div className="w-full text-center">
      <div className="flex justify-between mb-2">
        <button onClick={() => handleYearChange(-1)} className="text-gray-600 hover:text-black">«</button>
        <button onClick={() => handleMonthChange(-1)} className="text-gray-600 hover:text-black">‹</button>
        <div className="font-semibold">{months[currentMonth]} {currentYear}</div>
        <button onClick={() => handleMonthChange(1)} className="text-gray-600 hover:text-black">›</button>
        <button onClick={() => handleYearChange(1)} className="text-gray-600 hover:text-black">»</button>
      </div>
